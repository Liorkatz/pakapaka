const CACHE_NAME = 'pakapaka-1-01';
...
<script>
(function(){
  var VERSION='1.01', lastKey='pakapaka_last_shared_seen_v1', toastTimer=null, pullY=null, pullDone=false;
  ...
  function patch(){if(window.__v101)return;window.__v101=true;var oldR=window.refreshList;if(typeof oldR==='function')window.refreshList=async function(){var x=await oldR.apply(this,arguments);toast('עודכן');if(activeTab==='shared')seen();return x};var oldT=window.setTab;if(typeof oldT==='function')window.setTab=function(tab){var x=oldT.apply(this,arguments);if(tab==='shared')setTimeout(seen,700);return x};}
  ...
  function updateServiceWorker(){try{if(navigator.serviceWorker&&navigator.serviceWorker.controller){navigator.serviceWorker.getRegistration().then(function(reg){if(reg)reg.update()})}}catch(e){}}
  function init(){addVersion();dot();patch();pull();check();updateServiceWorker();setInterval(check,30000);window.addEventListener('focus',check)}
